...
main (int argc, char *argv[])
{
  ...
  BottleneckLink.SetQueue ("ns3::DropTailQueue", "MaxSize", StringValue ("18p"));
...
//
  ...
  tch1.Uninstall(devicesBottleneckLink);
...
//  Simulator::Schedule (Seconds (0.1), &RTTConnect);
  Simulator::Schedule (Seconds (0.02000001), &StateConnect);
...
//  Simulator::Schedule (Seconds (0.1), &GetQueue);
  ...
  BottleneckLink.EnableAsciiAll (ascii.CreateFileStream ("myfirst.tr"));
 // EnablePcap ("second11",devicesBottleneckLink.Get(0) );
