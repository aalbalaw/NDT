...
GetStats2 ()
{
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i =
	  ...
	  && t.destinationAddress == Ipv4Address ("10.0.2.2"))
	{
	 ...
	 return (i->second.timeLastRxPacket.GetSeconds()-i->second.timeFirstTxPacket.GetSeconds());
...
//
  ...
  sourceApps.Start (Seconds (startTime)); //SET START TIME RANDOMLY
  ...
  oss3 << "/NodeList/" << srcNode.Get (0)->GetId()<< "/$ns3::TcpL4Protocol/SocketList/0/CongState";
...
//  Simulator::Schedule (Seconds (0.1), &CwndConnect);
//  Simulator::Schedule (Seconds (0.1), &GetDelay);
 // Simulator::Schedule (Seconds (0.03), &StateConnect);
  ...
  std::cout << (sink1->GetTotalRx () * 8.0
  	  		  / GetStats2()) / 1024 / 1024<<std::endl;
  GetStats();
  std::cout << sink1->GetTotalRx ()/1000000.0 << std::endl;
