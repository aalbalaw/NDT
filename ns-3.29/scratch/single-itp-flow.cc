...
main (int argc, char *argv[])
{
  ...
  int coSize = 6000;
  std::vector<int>contentStore(coSize,0);
  populateArray (contentStore, coSize,5000);

  Ptr<Ipv4L3Protocol> ipv4Instance = gwNode2.Get(0)->GetObject<Ipv4L3Protocol> ();
  ipv4Instance->SetCache();

  Ptr<Ipv4L3Protocol> ipv4Instance2 = gwNode2.Get(0)->GetObject<Ipv4L3Protocol> ();
...
//  pem->SetList (sampleList);
 // srcNode.Get(0)->GetDevice(0)->SetAttribute ("ReceiveErrorModel", PointerValue (pem));
...
//  myq->TraceConnectWithoutContext ("DropID", MakeCallback (&DropID));
  ...
  BottleneckLink.EnableAscii (ascii.CreateFileStream ("myfirst1.tr"),devicesBottleneckLink );
  Simulator::Run ();
  GetStats();
  // GetStats2();
  Simulator::Destroy ();
