...
RequestResponseClientHelper::RequestResponseClientHelper (Address localDnsaddress, uint16_t port, bool enableDNS)
{
  m_factory.SetTypeId (RequestResponseClient::GetTypeId ());
...
void
RequestResponseClientHelper::SetURL (Ptr<Application> app, std::string url)
{
  app->GetObject<RequestResponseClient>()->SetURL (url);
}
