...
void BulkSendApplication::StartApplication (void) // Called at time specified by Start
{
 // //NS_LOG_FUNCTION (this);
	...
	NS_LOG_INFO("Start Time: "<< Simulator::Now().GetSeconds());
  ...
  if (!m_socket)
    {
     ...
     // //NS_LOG_INFO("\t "<<Simulator::Now().GetMilliSeconds()<<" Connect");
      m_socket->Connect (m_peer);
     // //NS_LOG_INFO("\t "<<Simulator::Now().GetMilliSeconds()<<" After Connect");
 ...
 //// //NS_LOG_FUNCTION (this);
  ...
  while (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    { // Time to send more
      ...
      //NS_LOG_INFO("Packet ID: "<<packet->GetUid());
      if (actual > 0)
        {
          m_totBytes += actual;
          m_txTrace (packet);
  ...
  if (m_totBytes == m_maxBytes && m_connected)
    {
//	  //NS_LOG_INFO("Time done: "<<Simulator::Now ().GetSeconds () << " s");
      m_socket->Close ();
      m_connected = false;
    }
}
...
void BulkSendApplication::ConnectionSucceeded (Ptr<Socket> socket)
{
 ...
 // //NS_LOG_LOGIC ("\t"<<Simulator::Now().GetMilliSeconds()<<" BulkSendApplication Connection succeeded");
  m_connected = true;
  SendData ();
}
