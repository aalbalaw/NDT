...
{
  static TypeId tid =
      TypeId ("ns3::FifoQueueDisc").SetParent<QueueDisc> ().SetGroupName (
	  "TrafficControl").AddConstructor<FifoQueueDisc> ().AddAttribute (
	  "MaxSize",
	  "The max queue size",
...
//          MyTag tagg;
//                if (p->RemovePacketTag (tagg))
//          	{
//          	  UdpHeader udpHdr;
//          	  p->RemoveHeader (udpHdr);
//
//          	  ItpHeader itpHdr;
//          	  p->RemoveHeader (itpHdr);
//
//          	  if (itpHdr.GetType () == 2)
//          	    {
//          	    //NS_LOG_INFO("\t"<<Simulator::Now().GetMilliSeconds()<<"\t"<< GetCurrentSize ().GetValue()<<"\t"<<itpHdr.GetSeqNumber());
//          	    NS_LOG_INFO("Dropping interest");
//          	    }
//          	  p->AddHeader (itpHdr);
//          	  p->AddHeader (udpHdr);
//
