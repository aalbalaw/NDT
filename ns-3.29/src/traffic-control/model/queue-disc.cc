...
TypeId QueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::QueueDisc")
   ...
   .AddTraceSource ("EnqueueID", "Enqueue a packet in the queue disc",
			MakeTraceSourceAccessor (&QueueDisc::m_traceEnqueueID),
			"ns3::TracedValueCallback::Uint64")
    .AddTraceSource ("Dequeue", "Dequeue a packet from the queue disc",
                     MakeTraceSourceAccessor (&QueueDisc::m_traceDequeue),
                     "ns3::QueueDiscItem::TracedCallback")
   .AddTraceSource ("DequeueID", "Enqueue a packet in the queue disc",
		      MakeTraceSourceAccessor (&QueueDisc::m_traceDequeueID),
		      "ns3::TracedValueCallback::Uint64")
    ...
    .AddTraceSource ("Drop", "Drop a packet stored in the queue disc",
                     ...
                     "ns3::QueueDiscItem::TracedCallback")
   .AddTraceSource ("DropID", "Enqueue a packet in the queue disc",
		      MakeTraceSourceAccessor (&QueueDisc::m_traceDropID),
		      "ns3::TracedValueCallback::Uint64")
...
QueueDisc::PacketEnqueued (Ptr<const QueueDiscItem> item)
{
  ...
  m_traceEnqueueID(item->GetPacket()->GetUid());
}
  ...
  if (!m_peeked)
    {
      ...
      m_traceDequeue (item);
      m_traceDequeueID(item->GetPacket()->GetUid());
    }
}
...
QueueDisc::DropBeforeEnqueue (Ptr<const QueueDiscItem> item, const char* reason)
{
  ...
  m_traceDropID( item->GetPacket()->GetUid());
}
