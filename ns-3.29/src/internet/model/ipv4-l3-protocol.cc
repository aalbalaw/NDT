// -*- Mode:C++; c-file-style:"gnu"; indent-tabs-mode:nil; -*-
 ...
 Ipv4L3Protocol::Ipv4L3Protocol()
 {
   //NS_LOG_FUNCTION (this);
	 m_imCache=false;
	 m_imCctpCache = false;
	 m_itpCache = CreateObject<ItpCache> ();
	 m_cctpCache = CreateObject<CctpCache> ();
 }
                           ...
                           const Address &to, NetDevice::PacketType packetType)
 {
   ...
   if(m_imCache){
	   NS_LOG_INFO("Packet Size before cache: "<<p->GetSize());

	   if(m_imCctpCache){
		   packet=m_cctpCache->CheckPacket(p->Copy());
	   }else{
		   packet=m_itpCache->CheckPacket(p->Copy());
	   }

	  NS_LOG_INFO("cache address: "<<device->GetAddress()<<" interface index: "<<device->GetIfIndex()<<" node id: "<<device->GetNode()->GetId());
   }else{
	   packet=p->Copy ();
   }
   ...
   if (ipHeader.GetPayloadSize () < packet->GetSize ())
     {
	//   NS_LOG_INFO("trimming: Payload Size: "<<ipHeader.GetPayloadSize()<<" packet size: "<<packet->GetSize());
 ...
 void
 Ipv4L3Protocol::SetCache(){
	 m_imCache=true;
	 m_itpCache->populateContentStore();
 }

 void
 Ipv4L3Protocol::SetCctpCache(std::string addr){
	 m_imCache=true;
	 m_imCctpCache= true;
	 m_cctpCache->populateContentStore(addr);
 }

 void
 Ipv4L3Protocol::SetCache(std::vector<int>contentStore){
 	 m_imCache=true;
 	 m_itpCache->populateContentStore(contentStore);
  }
 } // namespace ns3
