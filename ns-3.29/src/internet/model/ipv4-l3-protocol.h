...
public:
  ...
  Ptr<Packet> ReceiveCache (Ptr< Packet> p);

  void SetCache();

  void SetCctpCache(std::string addr);

  void SetCache(std::vector<int>contentStore);
...
private:
  ...
  bool m_imCache;
  bool m_imCctpCache;
  Ptr<ItpCache> m_itpCache;
  Ptr<CctpCache> m_cctpCache;
