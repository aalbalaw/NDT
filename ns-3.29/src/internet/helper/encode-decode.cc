 ...
 */
bool
StringToBuffer (std::string s, uint8_t *buffer)
{
  //
...
std::string ManifestToString(uint32_t numberOfChunk, Ipv4Address serverAddress){
	...
	std::string manifest = BufferToString (byteBuffer, size);

	delete byteBuffer;
	return manifest;
}

bool StringToManifest(ManifestHeader &mHdr, std::string ManifestAsString){
	...
	bool status = StringToBuffer(ManifestAsString, byteBuffer);
